...
public class MostFrequentCrimeTypeOf2016 {
	ArrayList<Crime> crime = new ArrayList<Crime>();
		
	public MostFrequentCrimeTypeOf2016(ArrayList<Crime> crime) {
			this.crime = crime;
	}
		 ...
		 * @author Tian Xue
		 * @param crimeData
		 * @return The most frequent crime type happened in Philly in 2016
		 */
	public String calMosFreCri(ArrayList<Crime> crimeData) {
		...
		for(Crime crime: crimeData) {
			...
			if(crime.getGeneralCode().equalsIgnoreCase(crimeType) && crime.getDispatchDate().contains("2016")) {
